...
def clustering_hits(mvtx_cluster_types, type_hits, hits_dict, pid_dic, p_dic):
    ...
    while hits_dict:
        ...
        #clustered_hit['ParentPDGID'] = p_dic[clustered_hit['pid']]['ParentPDGID']
...
def Parse_event(mvtx_cluster_types, filename, output_dir, file_number):
    ...
    for event in raw_data['Events']:
        ...
        print(f'Parsing {filename=} {file_number=} {event_ID=}')
        trigger = int(event['TruthTriggerFlag']['Flags']['D0toPiKInAcceptance'])
        ip = np.array(event["MetaData"]['CollisionVertex'])
        #pvx, pvy, pvz = event['TruthHit']['PVx'], event['TruthHit']['PVy'], event['TruthHit']['PVz']
        #ip = np.array([pvx, pvy, pvz])
        ...
        # TODO: modify so we actually read the trigger flag once Zhaozong gives us the updated dataset
        #trigger = 1
        ...
        pid_dic = {}
        p_dic = {np.nan: {'OriginVertexPoint': np.nan*np.ones(3), 'TrackMomentum': np.nan*np.ones(3), 'TrackEnergy':np.nan, 'ParticleTypeID':np.nan, 'TriggerTrackFlag':0, 'ParentPDGID':np.nan}}
        pid_dic['MVTX'] = {}
        pid_dic['INTT'] = {}
        ...
        for track in truth_tracks:
            #track['trigger_node'] = track['TrackType']
        ...
        # TODO: modify so from now on we actually read the trigger flag
        #valid_trigger_flag = 1

        for truth_track in truth_tracks:
            pid = truth_track['TrackSequenceInEvent']
            p_dic[pid] = {'OriginVertexPoint': truth_track['OriginVertexPoint'], 'TrackMomentum': truth_track['TrackMomentum'], 'TrackEnergy': truth_track['TrackEnergy'], 'ParticleTypeID': truth_track['ParticleTypeID'], 'TriggerTrackFlag': truth_track['trigger_node']}#, 'ParentPDGID': truth_track['ParentPDGID']}
        ...
        np.savez(output_dir_event, 
                ...
                #parent_particle_type=hits_df['ParentPDGID'].values.astype(np.float64),
...
def main():
    ...
    #TRIGGER_DATA_DIR = '/home/zhaozhongshi/HFMLTrigger/NewBBbarGood/JSON/'
    #TRIGGER_DATA_DIR = '/home/zhaozhongshi/HFMLTrigger/NewSamplePVOff/JSON/bbbar'
    #TRIGGER_DATA_DIR = '/home/zhaozhongshi/HFMLTrigger/NewSamplePVOff/JSON/MB'
    #trigger_output_dir = '/ssd1/giorgian/hits-data-august-2022-ctypes/trigger'
    trigger_output_dir = '/ssd3/giorgian/hits-data-august-2022-bp/trigger/'
    ...
    nontrigger_output_dir = '/ssd3/giorgian/hits-data-august-2022-bp/nontrigger/'
