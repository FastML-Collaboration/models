...
class HitGraphDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.n_mix == 1:
            x_intt, x_mvtx, edge_index, edge_index_intt, y, event_info, trigger_node = load_graph(self.filenames[index], self.cylindrical_features_scale, self.phi_slope_max, self.z0_max, use_intt=self.use_intt, construct_edges=self.construct_edges, drop_l1=self.drop_l1, drop_l2=self.drop_l2, drop_l3=self.drop_l3, add_global_node=self.add_global_node, apply_constraints=self.apply_constraints)
