...
def precessing_hits(event, type_hits):
    hits = []
    if type_hits == 'MVTX':
...
def clustering_hits(type_hits, hits_dict, pid_dic, p_dic):
    hits_clustered = []
    while hits_dict:
        ...
        clustered_hit['pixel_coordinates'] = cluster_coords
        hits_clustered.append(clustered_hit)
    hits_df=pd.DataFrame(hits_clustered)
    return hits_df

def cluster_hits_by_event(event, pid_dic, p_dic):
    hits_dict = {}
    hits_dict['MVTX'] = precessing_hits(event, 'MVTX')
    hits_dict['INTT'] = precessing_hits(event, 'INTT')
    len_mvtx = len(hits_dict['MVTX'])
    len_intt = len(hits_dict['INTT'])
#     if len_mvtx == 0 or len_intt == 0:
#         # TODO to confirm
#         return hits_dict
    list_pd = [clustering_hits(type_hits, hits_dict[type_hits], pid_dic, p_dic) for type_hits in ['MVTX', 'INTT']]
    hits_df = pd.concat(list_pd)
    # print(str(len_mvtx) + '->' + str(len(list_pd[0])) + '\t' + str(len_intt) + '->' + str(len(list_pd[1])))
...
def Parse_event(filename, output_dir, file_number):
    ...
    for event in raw_data['Events']:
        ...
        pixels = []
        pixel_hit_ids = []
        for i, activated_pixels in enumerate(hits_df['pixel_coordinates']):
            for pixel_coords in activated_pixels:
                pixels.append(pixel_coords)
                pixel_hit_ids.append(i)
        ...
        pixel_hit_ids = np.array(pixel_hit_ids)
        ...
        np.savez(output_dir_event, 
                ...
                pixel_cartesian=pixels.astype(np.float64),
                hit_id=pixel_hit_ids.astype(int),
        )
...
def main():
    ...
    output_dir = '/ssd2/giorgian/pixels-data-august-2022/trigger'
